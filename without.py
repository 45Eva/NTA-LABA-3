...
def form_linear_equations(alpha, n, factor_base):
    """Формування лінійних рівнянь для дискретних логарифмів."""
    t = len(factor_base)
    num_equations = t + 15
    equations = []
    ...
    while len(equations) < num_equations:
        ...
        if is_smooth(alpha_k, factor_base):
            ...
            print(f"Додано рівняння: {equation}")
        
        if k >= n:
            raise ValueError("Не вдалося знайти достатню кількість гладких чисел.")
    
    if len(equations) < num_equations:
        raise ValueError("Недостатньо рівнянь для формування необхідної кількості рівнянь.")
